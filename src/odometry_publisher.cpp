...
public:
  OdometryPublisher()
      : Node("odometry_publisher"),
        // すべてのエンコーダ変数を0で初期化する
        left_encoder_(0), right_encoder_(0), last_left_encoder_(0),
        last_right_encoder_(0), x_(0.0), y_(0.0), theta_(0.0)
...
private:
  ...
  void publish_odometry() {
    ...
    odom_msg.child_frame_id = "base_link"; // link

    // 速度を設定（今回は仮で0としていますが、実際はエンコーダの変化量から計算します）
    odom_msg.twist.twist.linear.x =
        delta_distance / 0.05; // 0.1秒周期なので0.1で割る
    ...
    odom_tf.child_frame_id = "base_link"; // link
