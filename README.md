...
| **docker** | コンテナ内のUIDがrootと同じになっている影響でコンテナ内で作成したファイルをマウントされているホストで編集しようとすると権限エラーになります。あとで直す|
...
コントローラで筐体を動かしながらマップを作成します。移動中は rviz2 上でも筐体が動いていることを確認してください（マップ作成にはLiDARだけでなくエンコーダの接続が必要です）。

#### 4. mapを作れたら、以下のコマンドを実行し、保存してください。<マップ名>は保存したいファイルの名称に変更してください。
...
#### 5. nav.launch.pyのデフォルトのパスを変更してください。
#### 6. navigation2を起動します。保存したマップと同じ場所で、スタート地点とゴール地点を定めて自律走行させることができます。
...
nav2 は起動直後、ロボットの正確な位置を把握していません。必ず初期位置を指定してください。

rviz2 上部ツールバーの「2D Pose Estimate」をクリック

地図上の、実際の筐体が存在する位置・向きをドラッグして指定
